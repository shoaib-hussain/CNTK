...
void TrainLSTMSequenceClassifer(const DeviceDescriptor& device, bool useSparseLabels, bool testSaveAndReLoad)
{
    ...
    auto trainer = CreateTrainer(classifierOutput, trainingLoss, prediction, { MomentumSGDLearner(classifierOutput->Parameters(), learningRatePerSample, momentumTimeConstant) });
